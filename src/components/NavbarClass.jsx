  ...
  return (
    <div className='border-b-2 dark:border-b-[#fffd92] border-gray-300 rounded-lg col-span-8 md:col-span-6 md:col-start-3 md:row-start-1'>
      <ul className='flex gap-5 h-full items-center justify-center md:justify-start p-2'>
        ...
        <li
          className={`dark:text-white w-fit px-2 cursor-pointer ${selectedView === 'announcements' ? 'border-b-2 border-blue-500 rounded-lg' : ''}`}
          onClick={() => onSelect('announcements')}
        >
          <h1 className='font-semibold'>Anuncios</h1>
        </li>
        <li
          className={`dark:text-white w-fit px-2 cursor-pointer ${selectedView === 'people' ? 'border-b-2 border-blue-500 rounded-lg' : ''}`}
